...
[list]
[*][b]目標：[/b]Windows 單人世界
[*][b]目前狀態：[/b]v0.5.18 唯一反向配對實測版；優先使用引擎直接提供的 Filter／Waza，若最終傷害先於來源事件抵達，只在同一攻擊者、同一 Boss 且候選傷害唯一時反向配對；多筆候選、逾時或來源衝突一律維持未辨識，不依傷害數字、倍率或目前動作猜測。F1 舊外部設定暫停，F2 安全歸零；原生 DLL 正式套件化與實機驗收仍在進行中
