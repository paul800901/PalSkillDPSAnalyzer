...
[h2]技能傷害、名稱與 v0.5.45[/h2]
[list]
[*]v0.5.45 在「分組百分比」頁的每個技能旁，同時顯示實際累計傷害與占比，不必再自行換算。
[*]公開名稱與遊戲內名稱改為「帕魯技能傷害計量器」，使名稱符合以傷害為主的顯示內容。既有 Workshop 訂閱、套件身分、設定與相容性均維持不變。
[*]v0.5.44 修正 48 隻帕魯等大型手動測試結束交戰後，CPU 仍持續偏高、FPS 無法恢復的問題。沒有新戰鬥資料時，遊戲執行緒不再每 0.5 秒從完整動作紀錄重建所有帕魯與技能明細。
[*]主面板與 F3 結果頁改為專注顯示累計傷害、占比、Hit 與施放次數；移除經過時間與所有 DPS 數值。
[*]v0.5.43 修正原生 F3 設定頁：切到 English 後，區段標題、欄位名稱與重設／關閉按鈕也會同步切換，不再保留中文。
...
[*][b]目前版本：[/b]v0.5.43。目標為 Windows 單人世界；需要 UE4SS Experimental。
[/list]
